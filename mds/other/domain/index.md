...
### 第一步：添加 TXT 记录，验证域名归属权

添加文件域名时，需要先通过 DNS 的 TXT 记录验证该域名的归属权，验证通过后方能添加域名。

以添加域名 `files.bmobpay.com` 为例，需要在域名解析处添加如下 TXT 记录：

| 主机记录 | 记录类型 | 记录值 |
| --- | --- | --- |
| `_dnsauth.files` | `TXT` | `650931f80b8595f8fd823b980e046f00` |

到域名解析那边添加该 TXT 记录后，再回到 Bmob 这边点击 **TXT 验证**；验证通过后，填入证书并点击 **保存**。
...
API域名也就是restful 域名，绑定教程与文件域名完全一致，在最后一步验证是否生效的操作下，可以访问自己绑定API域名与 自己备案域名 内容是否一致，一致则绑定成功。



## SDK域名绑定教程

SDK域名也就是APP SDK使用域名，绑定教程与文件域名完全一致，在最后一步验证是否生效的操作下，可以访问自己绑定SDK域名与 open2.bmobapp.com 内容是否一致，一致则绑定成功。
...
云函数域名也就是平时使用的cloud.bmobapp.com 域名，绑定教程与文件域名完全一致，在最后一步验证是否生效的操作下，可以访问自己绑定SDK域名与 cloud.bmobapp.com 内容是否一致，一致则绑定成功。
