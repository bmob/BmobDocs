## v2.6.0(2026-09-07)
1、文件上传改为经开放接口 `POST /8/files`（加密通道，与其它业务接口鉴权一致），不再由客户端直传又拍云
2、文件删除仍走 `POST /8/delcdnupload`，兼容阿里云 CDN 返回的 `cdn` 标识
3、`saveInBackgroundByDataSharding:` 兼容保留，实际与整文件上传相同
