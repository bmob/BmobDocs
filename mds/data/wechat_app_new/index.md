...
支持直接上传文本内容或 JSON 数据为文件，无需本地文件，适用于生成配置文件、日志文件、JSON 数据备份等场景。

**参数说明：**

| 参数     | 类型   | 必填 | 说明                   |
| -------- | ------ | ---- | ---------------------- |
| fileName | string | 是   | 文件名(带后缀，如 hello.txt、data.json) |
| content  | string | 是   | 文本内容或 JSON 字符串 |
...
```
const jsonData = { name: 'Bmob', version: '2.0' };
const jsonFile = Bmob.TextFile('data.json', JSON.stringify(jsonData));
jsonFile.save().then(res => {
  console.log('json file uploaded:', res[0]);
}).catch(err => {
  console.log('json file upload error:', err);
});
```
...
```
[{"cdn":"upyun","filename":"hello.txt","url":"http://…2026/04/30/abc123.txt"}]
```
